...
const RealtyEdit = ({ logined, setLogined }) => {
  ...
  const onrealtyInside = (data) => {
    const realty = [];
    data.map((insideRealty, index) => {
      let a = insideRealty.includes(",");
      let b;
      if (a) {
        b = insideRealty.slice(0, -2);
      } else {
        b = insideRealty;
      }

      for (let i = 0; i < Inside_List.length; i++) {
        if (b == Inside_List[i].data) {
          realty.push(b);
          break;
        } else {
          setInside(inside.filter((el) => el !== b));
        }
      }
    });
    setInside(realty);
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        setCategory(data.data.realtyCategory);
        setSpaceValue(data.data.realtySpace);
        setAreaValue(data.data.realtyArea);
        setRoomValue(data.data.realtyRoom);
        setBathValue(data.data.realtyBath);
        setAddressDetail(data.data.realtyAddress);
        setWholeValue(data.data.realtyWhole);
        setFloorValue(data.data.realtyFloor);
        setMove(data.data.realtyMove);
        setMoveDate(data.data.realtyMoveDate);
        setLoan(data.data.realtyLoan);
        setPet(data.data.realtyPet);
        setParking(data.data.realtyParking);
        setElevator(data.data.realtyElevator);
        onrealtyInside(data.data.realtyInside);
        setContentValue(data.data.realtyContent);
        setDepositValue(data.data.realtyDeposit);
        setMonthlyValue(data.data.realtyMonthly);
        setShortTerm(data.data.realtyShortTerm);
        setDepositChange(data.data.realtyDepositChange);
        setDepositChangePrice(data.data.depositChangePrice);
        setCostValue(data.data.realtyCostContent);
        setSalePrice(data.data.realtySalePrice);
        onDealingChange(data.data.realtyDealing);
        setIntroduce(data.data.realtyIntroduce);
        setAddress(data.data.realtyAddressDong);
